...
>
> **趁現在做這個步驟時，順手在日曆上設一個提醒。** 到期前不會有任何預警：
> Spotify 不會告訴你 token 的發行日期，這個 Worker 也沒有記錄你何時授權的，
> 所以第一個訊號就是某次按下去失敗了。抓早一週：
>
> ```bash
...
> ```
>
> 在授權當天執行，把結果放進日曆。這個提醒本身同時也就是「何時授權過」的紀錄。
...
1. **取得 URL 內容（Get Contents of URL）**
   ...
   - 標頭（Headers）：新增一個 — 鍵值 `Authorization`，值 `Bearer <SHORTCUT_SECRET>`
     （`Bearer` 後面有一個半形空格；值與步驟 4 設定的相同）。這是唯一一個必須完全正確的
     欄位 — 填錯的症狀見步驟 6 的排查說明。
   - 要求內文：不用管它。設成「無」或留在預設的 `JSON`（不加任何欄位）都可以，因為 Worker
     完全忽略請求內文 — 這個端點沒有任何東西需要呼叫方傳參數。
2. **取得辭典值（Get Dictionary Value）**
   - 取得：`數值`
   - 鍵值：`message`
   - 來源會自動接上前一個動作的結果，加好之後整句會讀成
     「在 **URL 內容** 中取得 `message` 的 **數值**」

   iOS 中文介面用的是「**辭**典值」，在動作清單搜「字典」找不到。
3. **顯示通知（Show Notification）**
   - 內容：選用上一步的**辭典值**變數（不是整包「URL 內容」，否則通知會顯示原始 JSON）
   - 標題：可留空 — 這個欄位不是必填
   - 附件：留空。這裡放東西不會壞，但也顯示不出什麼，因為那個值就是一段純文字。
   - 播放聲音：看個人喜好。如果你觸發時不會看螢幕，開著比較有用。

這個動作只有**內容**那一欄是重要的。第 2 個動作才是最容易漏掉的一步：少了它，通知會直接顯示
整串 `{"message":"...","ok":true,"outcome":"added",...}`，而不是那句訊息。

整個捷徑就是這三個動作，沒有任何條件判斷。要在 Apple Watch 上用的話，同樣這三個動作複製一份
即可，不需要改。
