...
通知裡的每個欄位（歌名／歌手、節目名稱／單集標題）最多顯示 **28 個欄寬**，超過就在尾端加上 `…`。

欄寬而非字數：中日韓是全角字算 2 欄，英數算 1 欄，所以 28 欄約等於 14 個中文字或 28 個英文字元，兩種語言的視覺長度一致。

28 這個值是照通知橫幅的實際寬度算的：一行大約 38-40 欄、兩行約 76-80 欄。兩個欄位各 28 欄，加上
`已加入喜愛：`（12 欄）與 ` - `（3 欄），最長也只有約 73 欄，即使兩邊都被截斷仍在兩行內；同時一般
長度的歌名（`Bohemian Rhapsody` 是 17 欄）完全不會被切掉。
